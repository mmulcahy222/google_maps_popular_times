...
banner = '''
    Results are saved in results.txt on the directory of this file
'''
print(banner)
#ssl
...
while 1:
	...
	query = input("Search for a location to display it's most visited hours> ")
	...
	for result_count in range(number_of_results):
		try:
			...
			for popular_times_day_array in popular_times:
				...
				for popular_times_hour in hour_popular_times_array:
					...
					#
					#
					#	THE ROWS ARE PRINTED OUT HERE
					#	IF YOU WANT TO ADD CODE TO A DATABASE OR WOULD LIKE ME TO DO IT, IT WILL BE HERE
					#
					#
					#
					text = '| {:^9} | {:^10} | {:>3} |'.format(day, hour, percentage_as_hour)
					print(text)
					write_to_file(file,text)				
